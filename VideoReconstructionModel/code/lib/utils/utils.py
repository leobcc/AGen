...
def get_camera_params(uv, pose, intrinsics):
    ...
    ray_dirs = F.normalize(ray_dirs, dim=2)   # Ray directions are normalized
...
def rgb_from_pretrained_uv(pretrained_uv, img):
    n_samples = pretrained_uv.shape[0]
    rgb = np.zeros((n_samples, 3))
    for i in range(n_samples):
        uv_y, uv_x = pretrained_uv[i]
        lower_uv_x = np.floor(uv_x).astype(np.int32)
        lower_uv_y = np.floor(uv_y).astype(np.int32)
        upper_uv_x = lower_uv_x + 1
        upper_uv_y = lower_uv_y + 1
        decimal_x = uv_x - lower_uv_x
        decimal_y = uv_y - lower_uv_y
        rgb[i] = img[lower_uv_y, lower_uv_x] * (1 - decimal_x) * (1 - decimal_y) + \
                 img[lower_uv_y, upper_uv_x] * (1 - decimal_x) * decimal_y + \
                 img[upper_uv_y, lower_uv_x] * decimal_x * (1 - decimal_y) + \
                 img[upper_uv_y, upper_uv_x] * decimal_x * decimal_y

    return rgb

def get_uv_rgb_values(image_tensor, uv):
    # Ensure uv has shape [N, 2] where N is the number of points
    uv = uv.view(-1, 2)

    # Extract x and y coordinates
    x_coords = uv[:, 0].to(torch.long)
    y_coords = uv[:, 1].to(torch.long)

    # Index the image tensor to get RGB values at specified coordinates
    rgb_values = image_tensor[0, :, y_coords, x_coords]
